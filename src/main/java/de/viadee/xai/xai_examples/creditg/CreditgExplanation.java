 ...
 */
public class CreditgExplanation {
    private static final Logger LOGGER = LoggerFactory.getLogger(CreditgExplanation.class);


    public static void main(String[] args) throws IOException {
        // Load dataset and its description
        final AnchorTabular anchorTabular = CreditgDataset.createTabularTrainingDefinition();

        // Our first model to be explained (GBM). Imported from an H2O model pre-built and trained in R
        final Creditg_H2OModelWrapper h2oModel = new Creditg_H2OModelWrapper();
        ...
        h2oBuilder.setTau(0.95).setBeamSize(10);
    ...
    private static void outputTestsetAccuracy(String name, Function<TabularInstance, Integer> predictFunction) {
        final TabularInstance[] testData = CreditgDataset.createTabularTestDefinition().getTabularInstances();
        final int[] actualTestLabels = CreditgDataset.readTestLabels();
        int matches = 0;
