 ...
 */
public class SpambaseExplanation {
    private static final Logger LOGGER = LoggerFactory.getLogger(SpambaseExplanation.class);


    public static void main(String[] args) throws IOException {
        // Load dataset and its description
        final AnchorTabular anchorTabular = SpambaseDataset.createTabularTrainingDefinition();

        // Our first model to be explained (GBM). Imported from an H2O model pre-built and trained in R
        final Spambase_H2OModelWrapper h2oModel = new Spambase_H2OModelWrapper();
        ...
        h2oBuilder.setTau(0.7).setBeamSize(2);
    ...
    private static void outputTestsetAccuracy(String name, Function<TabularInstance, Integer> predictFunction) {
        final TabularInstance[] testData = SpambaseDataset.createTabularTestDefinition().getTabularInstances();
        final int[] actualTestLabels = SpambaseDataset.readTestLabels();
        int matches = 0;
